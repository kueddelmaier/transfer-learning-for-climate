...
def plot_r2_map(x, xhat, xhatexp, out_dir, split, hist = True, pdf=True):
...
def plot_r2_map_xhatexp_only(x, xhatexp, out_dir, split, pdf=True, plot_map = True):
    map_xxhatexp = compute_r2_map_3d(x, xhatexp)  
    if plot_map:
        plot_single_r2_map(map_xxhatexp,out_dir, split, pdf=True)
    return map_xxhatexp


def plot_single_r2_map(map_xxhatexp, out_dir, split, pdf):
    ...
    pcm = ax.pcolormesh(np.squeeze(map_xxhatexp), cmap=current_cmap, 
        vmin = min_val, vmax = max_val)
    ax.axis('off')
    ax.set_title('R^2(x, xhatexp)')

    fig.colorbar(pcm, ax=ax, extend='both', shrink=.7)
