...
save_path = '/home/jkuettel/latent-linear-adjustment-autoencoders/python/Ridge_model/data/eval_dez_2022_np'
...
def main():
    ...
    for ho_name in ho_names:
        ...
        ho_list.append(holdout(ho_name, pr, psl[:, :TOP_PC], pr.shape[0], HEIGHT, WIDTH))
    ...
    if DEBUG:
        ...
        #generate loc touples (so compatible with parallel implementation)
        loc_touples =  [(i,j) for i in range(WIDTH) for j in range(HEIGHT)]


    
  
    for i, loc in enumerate(loc_touples):
        if i % 100 == 0:
            print(i)
